...
test("release version and updater trust stay aligned across the desktop bundle", () => {
  ...
  expect(ALIASMODE_VERSION).toBe("0.1.0-beta.50");
  ...
  for (const contract of [
    ...
    "Start-NativeNetworkObservation",
    '"chromium-lifecycle"',
    '"firefox-lifecycle"',
    "nativeNetworkObservation = $true",
  ...
  for (const contract of [
    ...
    "matchedRoots.Enqueue",
    "native network observation did not become ready",
    "samples = $samples",
