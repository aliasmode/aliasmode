...
if (!origin || !["write", "verify", "close", "hold"].includes(mode)) throw new Error("Firefox bridge smoke runner input is missing");
...
try {
  ...
  if (mode === "hold") {
    ...
    try { await context.close(); } catch { protectedContext = true; }
    assert.equal(protectedContext, true, "the external runner cannot close the owner context");
    process.stdout.write("{\"protectedContext\":true}");
  } else {
    if (mode === "write") {
